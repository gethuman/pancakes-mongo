...
var IdxInfoModel;                                   // idx model (set in init())
...
var cache = { models: {}, idxInfo: {} };            // cached models and index info
...
function init(config) {
    ...
    connect(config.mongo.url, config.mongo.debug, config.mongo.mongos)
        .then(function () {

            IdxInfoModel = getModel({
                name: 'idx',
                fields: {
...
MongoAdapter.prototype.baseFind = MongoAdapter.prototype.find = function find(req) {
    ...
    var options = {
        skip: parseInt((req.skip || 0) + '', 10),  // hack to make sure we are setting an int
        lean: true
    };
    ...
    if (!req.findOne && !req.allStatuses && this.resource.fields &&
        this.resource.fields.status && !where.status && !where._id) {

        where.status = { $in: ['created', 'approved'] };
    }

    Q.fcall(function () {
        return setIndexInfo(name, where, req.sort);
    })
        .catch(function (err) {
            /* eslint no-console:0 */
            console.error(err);
    ...
    promise.then(function (data) {
        ...
        // if query takes more than 75 milliseconds, log it
        ...
        if (timeElapsed > 300) {
            ...
            /* eslint no-console:0 */
            console.error('------------\n' +
                'SLOW QUERY! ' + timeElapsed + 'ms for ' + name +
...
MongoAdapter.prototype.baseUpdate = MongoAdapter.prototype.update = function update(req) {
    ...
    // as long as straight update and not upsert, increment version
    if (!options.upsert) {
        data.$inc = data.$inc || {};
        data.$inc.__v = 1;
    }
    ...
    Q.fcall(function () {
        return setIndexInfo(name, where);
    })
        .catch(function (er) {
            /* eslint no-console:0 */
            console.error(er);
